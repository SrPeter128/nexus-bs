...
<title>Nexus-BS {{PRODUCT_VERSION_TAG}} Dashboard</title>
<style>
/* ── Reset ── */
...
:root{
  ...
  --muted:   #6f7d78;
  --sidebar: #0b0d0e;
  --sidebar-border: #22292b;
  --card-shadow: 0 1px 0 rgba(255,255,255,0.03), 0 8px 24px rgba(0,0,0,0.18);
  --r: 8px;
...
[data-theme="light"]{
  --bg:#f3f5f4;--bg2:#ffffff;--bg3:#e8edeb;--bg4:#d9e2df;
  --border:#cad4d0;--border2:#aebdb8;
  --accent:#007f73;--accent2:#376f86;--warn:#9b6500;--danger:#b73a3a;
  --text:#10201c;--text2:#40524d;--text3:#7a8b85;--muted:#7a8b85;
  --sidebar:#121817;--sidebar-border:#26312f;
  --card-shadow:0 1px 4px rgba(0,0,0,0.1);
}
[data-theme="blue"]{
  --bg:#080b0a;--bg2:#101512;--bg3:#17201b;--bg4:#213026;
  --border:#2f3b35;--border2:#4b5d54;
  --accent:#4ee18e;--accent2:#e4bd5c;--warn:#f0a33a;--danger:#f06464;
  --text:#edf3ee;--text2:#b2c3b8;--text3:#708174;--muted:#708174;
  --sidebar:#050706;--sidebar-border:#1f2923;
  --card-shadow:0 1px 0 rgba(255,255,255,0.04), 0 10px 28px rgba(0,0,0,0.28);
}
...
.btn:disabled,.btn.disabled{
  opacity:0.42;cursor:not-allowed;color:var(--text3);
  border-color:var(--border);background:var(--bg3);filter:grayscale(1);
}
.btn:disabled:hover,.btn.disabled:hover{
  color:var(--text3);border-color:var(--border);background:var(--bg3);
}
...
.about-title{
  font-size:24px;font-weight:750;color:var(--text);
...
.about-chip{
  font-family:var(--mono);font-size:10px;font-weight:700;
  color:var(--accent);border:1px solid rgba(0,194,168,0.35);
  background:rgba(0,194,168,0.08);border-radius:6px;padding:5px 8px;
}
...
<nav id="sidebar">
  <div class="sidebar-logo">
    ...
    <div class="logo-text">
      <div class="logo-name">Nexus-BS</div>
      <div class="logo-sub">{{PRODUCT_VERSION_TAG}}</div>
  ...
  <div class="sidebar-nav">
    ...
    <div class="nav-item" onclick="showPage('about',this)" id="nav-about">
      <span class="nav-icon">i</span>
      <span class="nav-label" data-i18n="about">ABOUT</span>
    </div>
  </div>
  ...
  <div class="sidebar-footer">
    ...
    <div class="sidebar-copyright">
      <div class="cr-line">Nexus-BS Project {{PRODUCT_VERSION_TAG}}</div>
...
<div id="main">
  ...
  <div id="topbar">
    ...
    <div class="topbar-right">
      <div class="theme-picker">
        ...
        <button class="theme-btn" data-t="blue" onclick="setTheme('blue',this)">Ops</button>
      </div>
  ...
  <div id="content">
    ...
    <!-- Live TX DSP monitor — works on any SDR because the analysis is done on the
         complex baseband samples Nexus-BS generates internally, BEFORE they reach
      ...
      <div class="card">
        <div class="card-head">
          ...
          <div class="card-actions">
            ...
            <button class="btn disabled" id="update-btn" onclick="startUpdate()" disabled title="OTA update disabled for now">⬆ Update</button>
    ...
    <div class="page" id="page-system">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="info-row"><div class="info-key" data-i18n="sys_version">Nexus-BS Version</div><div class="info-val accent" id="sysVersion">—</div></div>
    ...
    <div class="page" id="page-about">
      <div class="about-hero">
        <div class="about-panel">
          ...
          <div class="about-chip-row">
            ...
            <span class="about-chip">Clause-scoped ETSI hardening</span>
          </div>
        </div>
        <div class="about-panel">
          <div class="about-kicker">STATUS</div>
          <div class="about-copy">
            ETSI EN 300 392-2 work is tracked clause by clause. This dashboard does not claim
            formal certification; official certification requires recognized conformance
      ...
      <div class="card">
        <div class="card-head">
          <div class="card-title">Project Credits</div>
        </div>
        <div class="card-body" style="padding:14px 18px">
...
const LANGS={
  en:{
    ...
    online_badge:'ONLINE',kick:'Kick',sds:'SDS',wap:'SDS WAP',
    ...
    sys_version:'Nexus-BS Version',sys_os:'OS',sys_config:'Active Config',
    ...
    sys_os:'OS',sys_version:'Nexus-BS Version',sys_config:'Active Config',
  ...
  ro:{
    ...
    confirm_restart:'Repornire Nexus-BS?\nToate apelurile active vor fi întrerupte.',
    confirm_shutdown:'Oprire Nexus-BS și oprire completă Linux host?\nRF și dashboard se opresc până la revenirea alimentării.',
    confirm_logout:'Deconectare?',
    saved:'✓ Salvat — repornire pentru aplicare.',save_fail:'✗ Salvare eșuată',conn_error:'Eroare de conexiune.',
    update:'⬆ Update',update_available:'Actualizare disponibilă',update_title:'Update OTA — Nexus-BS Project',
    update_disabled:'⬆ Update dezactivat',
    ...
    sys_os:'OS',sys_version:'Versiune Nexus-BS',sys_config:'Config Activ',
  ...
  de:{
    ...
    act_call_group:'Group voice',act_call_individual:'P2P simplex',
    act_call_p2p_simplex:'P2P simplex',act_call_p2p_duplex:'P2P duplex',act_sds:'SDS',
    online_badge:'ONLINE',kick:'Entfernen',sds:'SDS',wap:'SDS WAP',
    ...
    confirm_restart:'Nexus-BS neu starten?\nAlle aktiven Anrufe werden beendet.',
    confirm_shutdown:'Nexus-BS herunterfahren und Linux-Host ausschalten?\nRF und Dashboard bleiben aus, bis die Stromversorgung wiederhergestellt wird.',
    ...
    update:'⬆ Update',update_available:'Update verfügbar',update_title:'OTA-Update — Nexus-BS Project',
    update_disabled:'⬆ Update deaktiviert',
    ...
    sys_os:'OS',sys_version:'Nexus-BS-Version',sys_config:'Aktive Konfig',
  ...
  es:{
    ...
    act_call_group:'Group voice',act_call_individual:'P2P simplex',
    act_call_p2p_simplex:'P2P simplex',act_call_p2p_duplex:'P2P duplex',act_sds:'SDS',
    online_badge:'EN LÍNEA',kick:'Expulsar',sds:'SDS',wap:'SDS WAP',
    ...
    confirm_restart:'¿Reiniciar Nexus-BS?\nTodas las llamadas activas se interrumpirán.',
    confirm_shutdown:'¿Apagar Nexus-BS y el host Linux?\nRF y dashboard quedarán apagados hasta restaurar la alimentación.',
    confirm_logout:'¿Cerrar sesión?',
    saved:'✓ Guardado — reinicia para aplicar.',save_fail:'✗ Error al guardar',conn_error:'Error de conexión.',
    update:'⬆ Update',update_available:'Actualización disponible',update_title:'Actualización OTA — Nexus-BS Project',
    update_disabled:'⬆ Update desactivado',
    ...
    sys_os:'OS',sys_version:'Versión Nexus-BS',sys_config:'Config Activa',
  ...
  hu:{
    ...
    act_call_group:'Group voice',act_call_individual:'P2P simplex',
    act_call_p2p_simplex:'P2P simplex',act_call_p2p_duplex:'P2P duplex',act_sds:'SDS',
    online_badge:'ONLINE',kick:'Kizárás',sds:'SDS',wap:'SDS WAP',
    ...
    confirm_restart:'Újraindítja a Nexus-BS-t?\nAz összes aktív hívás megszakad.',
    confirm_shutdown:'Leállítja a Nexus-BS-t és a Linux hostot?\nAz RF és a dashboard a tápellátás visszaállításáig leáll.',
    ...
    update:'⬆ Frissítés',update_available:'Elérhető frissítés',update_title:'OTA frissítés — Nexus-BS Project',
    update_disabled:'⬆ Frissítés letiltva',
    ...
    sys_os:'OS',sys_version:'Nexus-BS verzió',sys_config:'Aktív konfig',
  ...
  zh:{
    ...
    act_call_group:'Group voice',act_call_individual:'P2P simplex',
    act_call_p2p_simplex:'P2P simplex',act_call_p2p_duplex:'P2P duplex',act_sds:'SDS',
    online_badge:'在线',kick:'踢下线',sds:'SDS',wap:'SDS WAP',
    call_group:'组呼',call_p2p_s:'P2P-S',call_p2p_d:'P2P-D',
    confirm_kick:'确定踢下 ISSI {issi}？\n终端将被注销并强制重新注册。',
    confirm_restart:'确定重启 Nexus-BS？\n所有正在进行的通话将被中断。',
    confirm_shutdown:'确定关闭 Nexus-BS 并关闭 Linux 主机？\nRF 和仪表盘会停止，直到重新上电。',
    confirm_logout:'确定注销吗？',
    saved:'✓ 已保存 — 重启后生效',save_fail:'✗ 保存失败',conn_error:'连接错误',
    update:'⬆ 更新',update_available:'有可用更新',update_title:'OTA 在线更新 — Nexus-BS Project',
    update_disabled:'⬆ 更新已禁用',
    ...
    sys_version:'Nexus-BS 版本',sys_os:'操作系统',sys_config:'当前配置',
...
function applyLang(){
  ...
  if(typeof markUpdateDisabled==='function')markUpdateDisabled();
}
...
// ── Render helpers ────────────────────────────────────────────────────────
function eeLabel(ms){
  const mode=ms.energy_saving_mode||0;
  if(!mode||mode===0)return '<span class="badge badge-dim" title="Energy Economy: StayAlive" style="font-size:9px">StayAlive</span>';
  ...
  const sleepFrames=['',1,2,5,8,17,71,359];
  const col=colors[mode]||'var(--text2)';
  const f=ms.energy_saving_frame, mf=ms.energy_saving_multiframe;
  const start=(f&&mf)?` F${f}/MF${mf}`:'';
  return `<span class="badge" title="Energy Economy ${labels[mode]} — sleep ${sleepFrames[mode]} TDMA frames${start}" style="background:color-mix(in srgb,${col} 12%,transparent);border-color:${col};color:${col};font-size:9px">${labels[mode]}${start}</span>`;
}
...
function renderStations(){
  ...
  tb.innerHTML=ms.sort((a,b)=>a.issi-b.issi).map(m=>{
    ...
    return`<tr>
      ...
      <td class="col-mobile-hide" style="text-align:center">${eeLabel(m)}</td>
      ...
      <td><button class="btn btn-sm" onclick="openSds(${m.issi})">${t('sds')}</button> <button class="btn btn-sm" onclick="sendWap(${m.issi})">${t('wap')}</button> <button class="btn btn-sm btn-danger" onclick="kickMs(${m.issi})">${t('kick')}</button></td>
    </tr>`;
  }).join('');
}
...
function sendWap(issi){wsSend({type:'wap',dest_issi:issi});}
...
const OTA_UPDATE_ENABLED=false;
function markUpdateDisabled(){
  ...
  if(btn){
    ...
    btn.textContent=t('update_disabled');
    btn.title='OTA update disabled for now';
  }
}
...
async function startUpdate(){
  if(!OTA_UPDATE_ENABLED){markUpdateDisabled();return;}
...
async function loadSystemInfo(){
  try{
    ...
    document.getElementById('sysVersion').textContent=
      (sysData.product_name&&sysData.product_version_tag)
        ? `${sysData.product_name} ${sysData.product_version_tag}`
        : (sysData.stack_version||'—');
...
async function addLiveSds() {
  ...
  try {
    const r = await fetch('/api/live-sds', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, repeat_count: repeat, protocol_id: 130, source_issi: 16777215 })
    });
    if (r.ok) {
