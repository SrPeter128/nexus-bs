...
impl<T: NetworkTransport> ControlWorker<T> {
    ...
    fn route_control_command(command: &ControlCommand) -> TetraEntity {
        match command {
            ControlCommand::SendSds { .. } | ControlCommand::SendRawSds { .. } | ControlCommand::SendStatus { .. } => TetraEntity::Cmce,
            ...
            ControlCommand::PowerOffHost => TetraEntity::Cmce,
...
mod tests {
    ...
    #[test]
    fn test_route_send_status_to_cmce() {
        let target = ControlWorker::<MockTransport>::route_control_command(&ControlCommand::SendStatus {
            handle: 3,
            source_ssi: 9999,
            dest_ssi: 2000001,
            dest_is_group: false,
    ...
    #[test]
    fn test_route_power_off_host_to_cmce() {
        let target = ControlWorker::<MockTransport>::route_control_command(&ControlCommand::PowerOffHost);
        assert_eq!(target, TetraEntity::Cmce);
    }
